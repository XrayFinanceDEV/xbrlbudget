...
- **`sp_overrides` clampa a zero i valori negativi** (tranne `sp13_utile_perdita` e
  ...
  **squilibra il foglio** invece non si chiude più con una cassa a zero: sul previsionale passa dallo
  stesso cancello del plug (solleva, o diventa scoperto se concesso), e il ricalcolo finale della
  cassa (`_normalize_balance_sheet_cents(recompute_cash=True)`) **non scavalca più** quel cancello —
  era il difetto §11.1 della spec del lotto, che persisteva `sp09` negativa sotto un
...
way out is an explicit financing assumption, never a retry. **On:** the requirement becomes
`sp16a_debiti_banche_breve` **generated by the plan**, kept apart in `details` from pre-existing bank
debt (`scoperto_generato`, `scoperto_residuo`), with interest charged into `ce15` on the **opening**
overdraft (`oneri_scoperto`, at `financing_interest_rate`) — never on the overdraft the year itself
generates, which would be circular — and an optional cap, `overdraft_limit`, above which the engine
raises again. Cash generated in later years repays the overdraft first. The gate lives in one place,
`_Overdraft.copri` (`calculations/forecast_engine.py`). **Why it exists:** a stressed plan is
...
- **This engine is not the budget engine on two points that change the balance sheet.** Capital and
  ...
  un avviso sull'infrannuale e in un errore secco sul previsionale.
