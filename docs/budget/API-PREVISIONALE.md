...
omonomo di `tests/test_bulk_tipizzato.py`. Sono invece **tre** i payload dei test esistenti che
l'input-tipizzato rifiuta per un'altra ragione, e sono i tre nominati nel task (un tetto di scoperto
negativo, un `forecast_year` "abc", una differenza temporanea senza `name`).
...
nome **finisce per `_override`** (`budget_scenarios.py:1004-1007`). `sp_overrides` finisce per
...
- **Valore** = uno dei **tre driver**, tipizzato `Literal["ricavi", "acquisti", "personale"]`
  ...
  **lo rifiuta la porta normale dal lotto 3A**: il bulk `PUT /scenarios/{id}/assumptions` valida
  ...
  (`calculations/forecast_engine.py:850-851`). Quel motivo non è morto: lo percorre ancora chi
